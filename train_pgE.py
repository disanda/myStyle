...
def train():
	...
	#batch_size = 7
	batch_size_pg = [256,256,128,128,64,64,32,12,6]
	...
	for i,j in zip(range(9),progressive):
		for epoch in range(50000*j):
			...
			const2,w2 = E(imgs1.cuda(),i)

			with torch.no_grad():
				imgs2=Gs.forward(w2,i)
			...
			print('i_'+str(i)+'--epoch_'+str(epoch)+'--loss_all__:'+str(loss_all.item())+'--loss_1:'+str(loss_1.item())+'--loss_2_1:'+str(loss_2_1.item())+'--loss_2_2:'+str(loss_2_2.item())+'--loss_c:'+str(loss_c.item())+'--loss_kl_c:'+str(loss_kl_c.item()))
			...
			if epoch % 1000 == 0:
				...
				torchvision.utils.save_image(test_img, resultPath1_1+'/i_%d_ep%d.png'%(i,epoch), nrow=3)
				with open(resultPath+'/Loss.txt', 'a+') as f:
					print('i_'+str(i)+'--epoch_'+str(epoch)+'--loss_all__:'+str(loss_all.item())+'--loss_1:'+str(loss_1.item())+'--loss_2_1:'+str(loss_2_1.item())+'--loss_2_2:'+str(loss_2_2.item())+'--loss_c:'+str(loss_c.item())+'--loss_kl_c:'+str(loss_kl_c.item()),file=f)
					#print('loss_w_1:'+str(loss_w_1.item())+'--loss_w_2:'+str(loss_w_2.item())+'--loss_w_3:'+str(loss_w_3.item())+'--loss_w_2:'+str(loss_w_3.item()),file=f)
				if epoch % 10000 == 0:
					torch.save(E.state_dict(), resultPath1_2+'/E_model_i%d_ep%d.pth'%(i,epoch))

if __name__ == "__main__":
	resultPath = "./result/EB_V3_pgE"
